...
func LoadAllPrices(cfg *models.Config, site *models.Site, itemIndex models.ItemIndex) error {
	...
	for len(slugs) > 0 {
		...
		err := loadPrices(cfg, site, s, itemIndex)
		if err != nil {
			return err
		}
	}
...
func loadPrices(cfg *models.Config, site *models.Site, slugs []string, itemIndex models.ItemIndex) error {

	ids := strings.Join(slugs, ",")
	path := fmt.Sprintf("%s/services/pricing/v2/prices/show_multiple?items=%s", cfg.SiteURL, ids)
...
func processPrices(details prices, site *models.Site, itemIndex models.ItemIndex) error {
